$(document).ready(function() {
    ...
    PageDoc.prototype.init = function(file) {
        ...
        $("#" + this.editor)
            ...
            .on('click', '.sync', function(ev) {
                ...
                let nodes = $("[data-name]").filter("[data-id]").each(function() {
                    let name = $(this).data('name');
                    container.push(`<${name} id="${this.dataset.id}">${this.innerHTML}</${name}>\n`);
                });
                if (self.basetag) container.push(`</${self.basetag}>`);
    ...
    PageDoc.prototype.render = function(target) {
        ...
        let self = this;
        //1. if a tag exists with data-name={this.tagName}, the append the data
